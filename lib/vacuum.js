		...
		try {
			...
			if (answer.result !== 'unknown_method') {
				...
				if(maps.length >0){
					...
					adapter.log.debug("Ich war hier ");

					for(var propertyName in stateArray) 
					{
						adapter.log.debug("Und hier auch!");
						const result = await this.Miio.sendMessage('load_multi_map', new Array(parseInt(propertyName)));
						adapter.log.info("rooms: " + JSON.stringify(result));
					};
